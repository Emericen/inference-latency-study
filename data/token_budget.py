...
def _multi_image_template(image_token_counts: list[int], question: str) -> str:
    vision_tokens = "".join(
        "<|vision_start|>"
        + ("<|image_pad|>" * image_token_count)
        + "<|vision_end|>"
        for image_token_count in image_token_counts
    )
    return (
        "<|im_start|>user\n"
        + vision_tokens
...
def count_multi_image_prompt_tokens(
    tokenizer,
    image_processor,
    ...
    for image in images:
        ...
        image_token_count = int(
            image_grid_thw.prod() // (image_processor.merge_size**2)
        )
        image_token_counts.append(image_token_count)

    prompt = _multi_image_template(
        image_token_counts=image_token_counts,
        question=question,
    )
    total_prompt_tokens = len(tokenizer(prompt, add_special_tokens=False)["input_ids"])
    return total_prompt_tokens, image_token_counts
